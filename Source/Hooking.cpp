...
const char *hkGetLabelText(void *this_, const char *label)
{
	...
	{
		return "Loading Online With Hydra";
	}
	if (std::strcmp(label, "HUD_QUITTING") == 0)
	{
		return "Leave Online With Hydra";
	}
	...
	{
		return "Join Online with Hydra";
	}
	if (std::strcmp(label, "PM_EXIT_GAME") == 0)
	{
		return "Exit Game with Hydra";
	}
	if (std::strcmp(label, "PM_GO") == 0)
	{
		return "GO Online with Hydra";
	}
	if (std::strcmp(label, "PM_GOTO_STORE") == 0)
	{
		return "GOTO STORE with Hydra";
	}
	if (std::strcmp(label, "PM_FRIEND_FM") == 0)
	{
		return "Join Friend with Hydra";
	}
	if (std::strcmp(label, "PM_FIND_SESS") == 0)
	{
		return "Find New Session with Hydra";
	}
	...
	if (player != PLAYER::PLAYER_ID() && player != PLAYER::PLAYER_PED_ID()) {
		if (Hooking::e_weather_time) {
			if (eventId == GAME_CLOCK_EVENT) {
				if (Hooking::e_notify) {
					ostringstream oss; oss << "~d~" << PLAYER::GET_PLAYER_NAME(Features::index_to_id(player)) << " tried to change the time.";
					notifyMap(oss.str());
				}
		...
		if (Hooking::e_control) {
			if (eventId == REQUEST_CONTROL_EVENT || eventId == GIVE_CONTROL_EVENT || eventId == VEHICLE_COMPONENT_CONTROL_EVENT) {
				if (Hooking::e_notify) {
					ostringstream oss; oss << "~d~" << PLAYER::GET_PLAYER_NAME(Features::index_to_id(player)) << " tried to control you.";
					notifyMap(oss.str());
				}
		...
		if (Hooking::e_weapon) {
			if (eventId == GIVE_WEAPON_EVENT) {
				if (Hooking::e_notify) {
					ostringstream oss; oss << "~d~" << PLAYER::GET_PLAYER_NAME(Features::index_to_id(player)) << " tried to give you weapons.";
					notifyMap(oss.str());
				}
			...
			if (eventId == REMOVE_WEAPON_EVENT || eventId == REMOVE_ALL_WEAPONS_EVENT) {
				if (Hooking::e_notify) {
					ostringstream oss; oss << "~d~" << PLAYER::GET_PLAYER_NAME(Features::index_to_id(player)) << " tried to remove your weapons.";
					notifyMap(oss.str());
				}
		...
		if (Hooking::e_ptfx) {
			if (eventId == NETWORK_PTFX_EVENT) {
				if (Hooking::e_notify) {
					ostringstream oss; oss << "~d~" << PLAYER::GET_PLAYER_NAME(Features::index_to_id(player)) << " tried to show particles.";
					notifyMap(oss.str());
				}
		...
		if (Hooking::e_stat) {
			if (eventId == NETWORK_BANK_REQUEST_EVENT || eventId == NETWORK_INCREMENT_STAT_EVENT) {
				if (Hooking::e_notify) {
					ostringstream oss; oss << "~d~" << PLAYER::GET_PLAYER_NAME(Features::index_to_id(player)) << " tried to change your stats.";
					notifyMap(oss.str());
				}
		...
		if (Hooking::e_report) {
			if (eventId == PLAYER_CARD_STAT_EVENT || eventId == REPORT_MYSELF_EVENT || eventId == REPORT_CASH_SPAWN_EVENT) {
				if (Hooking::e_notify) {
					ostringstream oss; oss << "~d~" << PLAYER::GET_PLAYER_NAME(Features::index_to_id(player)) << " tried to report you.";
					notifyMap(oss.str());
				}
		...
		if (Hooking::e_kick) {
			...
			if (eventId == REMOTE_SCRIPT_LEAVE_EVENT || eventId == KICK_VOTES_EVENT) {
				if (Hooking::e_notify) {
					ostringstream oss; oss << "~d~" << PLAYER::GET_PLAYER_NAME(Features::index_to_id(player)) << " tried to kick you.";
					notifyMap(oss.str());
				}
		...
		if (Hooking::e_explosion) {
			if (eventId == FIRE_EVENT || eventId == EXPLOSION_EVENT || eventId == REQUEST_PHONE_EXPLOSION_EVENT || eventId == BLOW_UP_VEHICLE_EVENT) {
				if (Hooking::e_notify) {
					ostringstream oss; oss << "~d~" << PLAYER::GET_PLAYER_NAME(Features::index_to_id(player)) << " tried to explode you.";
					notifyMap(oss.str());
				}
		...
		if (Hooking::e_freeze) {
			if (eventId == RAGDOLL_REQUEST_EVENT || eventId == INCIDENT_ENTITY_EVENT || eventId == CLEAR_AREA_EVENT || eventId == CLEAR_RECTANGLE_AREA_EVENT || eventId == NETWORK_CLEAR_PED_TASKS_EVENT || eventId == NETWORK_SOUND_CAR_HORN_EVENT || eventId == NETWORK_PLAY_SOUND_EVENT || eventId == NETWORK_STOP_SOUND_EVENT || eventId == NETWORK_PLAY_AIRDEFENSE_FIRE_EVENT) {
				if (Hooking::e_notify) {
					ostringstream oss; oss << "~d~" << PLAYER::GET_PLAYER_NAME(Features::index_to_id(player)) << " tried to freeze you.";
					notifyMap(oss.str());
				}
