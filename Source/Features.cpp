...
Object AttachObjectToVehicleByHash(Hash hash, float x, float y, float z, float rotx, float roty, float rotz, Vehicle veh = PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), 0)) {
	if (!PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), 1)) {
		//CMenu::notifyBottom("~d~You need to be in a vehicle to load a preset !");
		return -1;
	}
...
void WriteVehicle(std::string name, Vehicle veh) {
	std::string iniPath = ".\\Hydra\\Vehicles\\" + name + ".ini";
...
void WritePreset(std::string name, float id, float x, float y, float z, float rotx, float roty, float rotz, Hash model) {
	...
	if (DoesFileExists(iniPath)) {
		//CMenu::notifyBottom("~d~You already have a preset with this name");
		return;
	}
...
std::vector<__int64> _eventsID = { 1000837481, 994306218, 713068249, 769347061, -15432926, -181444979, -1571039706, -1190833098, 360381720, 4119492672, };
std::vector<const char*> _eventsID_char = { "Teleport to apartments/Blackscreen", "Non - Kick Player", "Specatating - Message", "Kick to Singleplayer", "Send to Mission", "GTA V Banner Screen", "Mission Message", "Ceo Kick Player", "Ceo Ban Player", "Sound Spam", };
...
void Features::Stealth(bool toggle) {
	if ((timeGetTime() - Features::TimePD3) > Features::stealthdelay)
	{
		...
		notifyMap("~d~Bank~w~Transaction Active");
	}
}
...
void Features::RockstarAdminCheck()
{
	if (NETWORK::NETWORK_IS_SESSION_ACTIVE())
	{
		...
		for (int i = 0; i < 32; i++)
		{
			...
			for (auto id : Admins)
			{
				if (UserID = id)
				{
					notifyMap("~d~admin found in session, it's recommended you leave the session immediately!");
	...
	{
		notifyMap("~d~You must be online to use this function.");
...
void Features::Gay(bool toggle) {
	if ((timeGetTime() - Features::TimePD3) > Features::stealthdelay)
	{
		...
		notifyMap("~d~Wallet~w~Transaction Active");
	}
}
...
void Features::paranormal(bool toggle)
{
	if (GetAsyncKeyState(VK_NUMPAD9))
	{
		...
		UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG_2("CHAR_CALL911", "CHAR_CALL911", 1, 8, "~w~Hydra MENU", "~w~GTA", 1, "Notification", 9, 0);
...
{
	globalHandle(2531497).At(238).As<int>() = 1;
}
...
void Features::magnetActive(bool toggle) {
	...
	if (vehHash == 0xFCFCB68B || vehHash == 4244420235 || vehHash == -50547061) {
		if (VEHICLE::_DOES_CARGOBOB_HAVE_PICKUP_MAGNET) {
			...
			notifyBottom("~d~No magnet attached.");
		}
	}
	else {
		notifyBottom("~d~You're not in a Cargobob.");
	}
}
...
void Features::NotifyCenter()
{
	...
	GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(handle, "SHOW_SHARD_WASTED_MP_MESSAGE");
	GRAPHICS::_ADD_SCALEFORM_MOVIE_METHOD_PARAMETER_STRING("~d~Hydra");
	GRAPHICS::_ADD_SCALEFORM_MOVIE_METHOD_PARAMETER_STRING("~d~Welcome To Hydra");
...
void Features::freeCamt(bool toggle)
{
	...
	notifyBottom("~d~Choose a point and disable");
...
void Features::Superman(bool toggle)
{
	...
	notifyBottom("RUNNING IN THE 90S");
...
{
	notifyBottom("~d~Open Network Options *...");
...
int Features::stealthAmount = 100000000000;
...
Object ropes;
Vector3 handcoords;
int teleportActiveLineIndex = 0, bone = 0, onec = 0, MaxRadius = 8, MarkerRed = 255, marker
= 60, markerblue = 0;
bool attach5 = false;
...
void Features::lolop(bool toggle)
{
	...
	if ((timeGetTime() - Features::TimePD5) > 1000) { // Delay
		...
		int iVar0;
		STATS::STAT_GET_INT(GAMEPLAY::GET_HASH_KEY("MP0_MONEY_EARN_JOBS"), &iVar0, -1);
		STATS::STAT_SET_INT($("MP0_MONEY_EARN_JOBS"), iVar0 + Features::amount405, 1); notifyMap("~d~Click Numpad9!");
...
static void xex(Ped ped, float x, float y, float z, int explosionType, float damageScale, BOOL isAudible, BOOL isInvisible, float cameraShake)
{
...
void blame()
{
	...
	for (int i = 0; i < 32; ++i)
	{
		if (i != local && i != player)
		{
			...
			xex(ped, pos.x, pos.y, pos.z, 29, 20.f, true, false, 5.f);
		}
	}
...
/*bool Features::fcbool2 = false;
void Features::FlyingCarLoop2(bool toggle) {
	if (flyToggle) {
		Ped playerPed = PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), 0);
...
(GetAsyncKeyState(VK_SHIFT) || CONTROLS::IS_DISABLED_CONTROL_JUST_PRESSED(2, ControlFrontendRb)) {
	float heading = ENTITY::GET_ENTITY_HEADING(playerPed);
	ENTITY::SET_ENTITY_HEADING(playerPed, heading);
...
if (GetAsyncKeyState(VK_SPACE) || CONTROLS::IS_DISABLED_CONTROL_JUST_PRESSED(2, ControlFrontendLb)) {
	...
	bool Features::flyask = false;
	void Features::Noclipcar(bool toggle) {
	...
	void Features::Sticky(bool toggle)
	{
		if (PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), 0))
		{
...
void Features::container_rider(bool toggle) {
	...
	if (!PED::IS_PED_ON_ANY_BIKE(PLAYER::PLAYER_PED_ID())) {
		if (PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), 0)) {
			notifyBottom("~d~Container rider requires a bike.");
			return;
		}
	}
...
void Features::Kickprotec(bool toggle)
{
	...
	STATS::PLAYSTATS_IDLE_KICK(9999999999999999);
}
...
void Features::OffRadar(bool toggle)
{
	...
	notifyMap("~w~Off The Radar Is Active");
...
void Features::maxvehicle()
{
	...
	VEHICLE::SET_VEHICLE_NUMBER_PLATE_TEXT(vehicle, "Hydra MENU");
}
...
void Features::showDroppedMoney() {
	...
	if (showAmount && showAmount2) {
		...
		if (stealthMoneyCounter < 500000001 || droppedMoneyCounter < 2000001) {
			...
			ostringstream droppppppp;
			droppppppp << "~d~Cheated money: " << amount << "$";
			notifyBottom((char*)droppppppp.str().c_str());
		}
	}
	else if (showAmount) {
		...
		if (droppedMoneyCounter < 2000001) {
			...
			droppppppp << "~d~Dropped money: " << Features::droppedMoneyCounter << "$";
			notifyBottom(droppppppp.str());
		}
	}
	else if (showAmount2) {
		...
		if (stealthMoneyCounter < 500000001) {
			...
			droppppppp << "~d~Stealthed money: " << Features::stealthMoneyCounter << "$";
			notifyBottom(droppppppp.str());
		}
	}
}
...
bool Features::EventShield = false;
void Features::eventShield(bool toggle)
{
	globalHandle(1373500).At(591).As<int>() = 591; //Freemode
	globalHandle(1373500).At(506).As<int>() = 506; //Freemode
	globalHandle(1373500).At(33).As<int>() = 33; //Freemode
	globalHandle(1373500).At(63).As<int>() = 63; //Freemode

	globalHandle(1373500).At(506).As<int>() = 506; //Infinite Loading

	globalHandle(1373500).At(33).As<int>() = 33; //Sound Spam
...
void Features::LoadPlayerInfo(char* playerName, Player p) {
	...
	int rp = globalHandle(1590446).At(p, 871).At(211).At(5).As<int>();
	int earnings = globalHandle(1590446).At(p, 790).At(211).At(56).As<int>();
	int rank = globalHandle(1590446).At(p, 871).At(211).At(6).As<int>();
	int kills = globalHandle(1590446).At(p, 871).At(211).At(28).As<int>();
	int handle = globalHandle(1590446).At(p, 871).At(211).At(56).As<int>();
	int handle2 = globalHandle(1590446).At(p, 871).At(211).At(3).As<int>();
	int bankMoney = handle - 10 - handle2 + (5 * 1 + 3 + 2);
	int deaths = globalHandle(1590446).At(p, 871).At(211).At(29).As<int>();
	int tempMoney = globalHandle(1590446).At(p, 871).At(211).At(3).As<int>();
	int death = globalHandle(1590446).At(p, 871).At(211).At(20).As<int>();
	int deathlost = globalHandle(1590446).At(p, 871).At(211).At(21).As<int>();
	int races = globalHandle(1590446).At(p, 871).At(211).At(15).As<int>();
	int raceslost = globalHandle(1590446).At(p, 871).At(211).At(16).As<int>();
	float kd = globalHandle(1590446).At(p, 871).At(211).At(26).As<float>();
	...
	char buf[0x80]; snprintf(buf, sizeof(buf), "X: %f", __coords.x);
	char buf1[0x80]; snprintf(buf1, sizeof(buf1), "Y: ~g~%f", __coords.y);
	char buf2[0x80]; snprintf(buf2, sizeof(buf2), "Z: ~g~%f", __coords.z);
	...
	if (!NETWORK::NETWORK_IS_PLAYER_CONNECTED(p)) {
		Money << "Cash:~s~ N/A";
		Money1 << "Cash:~s~ $";
		Moneybank << "Bank Cash:~s~ $";
		RP << "RP: N/A";
		Rank << "Rank:~s~ N/A";
		Kills << "Kills:~s~ N/A";
		Deaths << "Deaths:~s~ N/A";
		KD << "KD Ratio:~s~ N/A";
		Earnings << "Total Earnings:~s~ N/A";
		Races << "Total Races Won:~s~~d~ N/A";
		Raceslost << "Total Races Lost:~s~~d~ N/A";
		Death << "Total Deathmatch Won:~s~~d~ N/A";
		Deathlost << "Total Deathmatch Lost:~s~~d~ N/A";
		//  //__ipbuf << "~w~ IP::~s~ N/A";
		// 
	}
	else {
		Money << "Cash: $" << money;
		Money1 << "Cash: $" << money2;
		Moneybank << "Bank Cash: $" << bankMoney;
		RP << "RP:" << rp;
		Rank << "Rank:" << rank;
		Kills << "Kills:" << kills;
		Deaths << "Deaths:" << deaths;
		KD << "KD Ratio:" << kd;
		Earnings << "Total Earnings: $" << earnings;
		Races << "Total Races Won: " << races;
		Raceslost << "Total Races Lost:" << raceslost;
		Death << "Total Deathmatch Won:" << death;
		Deathlost << "Total Deathmatch Lost:" << deathlost;
		...
		for (int i = 0; i < 3; i++)
		{
			if (nigger)
			{
				...
				if (faggot)
				{
					...
					std::snprintf(ipBuf, sizeof(ipBuf) - 1, "IP: ~s~Hide", i);
...
bool ESP::espjackass = false;
void ESP::esp(Player target)
{
	if (target != PLAYER::PLAYER_ID() && ENTITY::DOES_ENTITY_EXIST(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(target)))
	{
		Vector3 entitylocation = ENTITY::GET_ENTITY_COORDS(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(target), false);
...
bool ESP::ESPBox = false;
void ESP::espbox(Player target)
{
	std::string PlayerNameESP = PLAYER::GET_PLAYER_NAME(Features::Online::selectedPlayer);
...
void Features::joinMessage(bool toggle) {
	for (int i = 0; i < 33; i++) {
		if (!NETWORK::NETWORK_IS_PLAYER_CONNECTED(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(i)) && ENTITY::DOES_ENTITY_EXIST(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(i))) {
			notifyMap("~HUD_COLOUR_RED~ %s HAS JOINED.", PLAYER::GET_PLAYER_NAME(i));
		}
	}
}
int fam;
...
void Features::Speedometer(bool toggle) {
	...
	if (inVehicle) {
		...
		Speed << vehSpeedConverted << " ~w~KM/H";

		Menu::AddSmallInfo3((char*)Speed.str().c_str(), 5);
	}

}


bool Features::DEJI = false;
void Features::Logan(bool toggle) {
...
bool carweapons = false;
void CarWeapons(bool toggle)
{
	Ped playerPed = PLAYER::PLAYER_PED_ID();
	Vector3 pos = ENTITY::GET_ENTITY_COORDS(playerPed, false);
	ENTITY::SET_ENTITY_COORDS_NO_OFFSET(playerPed, pos.x, pos.y, pos.z, false, false, false);
...
{
	VEHICLE::SET_VEHICLE_FORWARD_SPEED(PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), 0), 0);
	notifyMap("~g~Press space to instant brake.");
}
...
void loggi()
{
	...
	else notifyMap("~d~You are not in any vehicle!");
}
...
void Features::cashdrop69(Player target)
{
	...
	{
		char* moneybags = "p_poly_bag_01_s";
		Hash DatHashDo = GAMEPLAY::GET_HASH_KEY(moneybags);
		Vector3 HewantsCash = ENTITY::GET_ENTITY_COORDS(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(Features::Online::selectedPlayer), false);
		STREAMING::REQUEST_MODEL(DatHashDo);
		if (STREAMING::HAS_MODEL_LOADED(DatHashDo))
		{
			OBJECT::CREATE_AMBIENT_PICKUP(GAMEPLAY::GET_HASH_KEY("PICKUP_MONEY_CASE"), HewantsCash.x, HewantsCash.y, HewantsCash.z + DropHeight, 0, DropAmount, DatHashDo, 0, 1);
			STREAMING::SET_MODEL_AS_NO_LONGER_NEEDED(DatHashDo);
